...
class QM7FileParser(BasicFileParser):
    def __init__(self):
        super(QM7FileParser, self).__init__()
...
import numpy as np
def ZscoreNormalization(Z):
    Z_mean = Z.mean(axis=0)
    Z_var = Z.var(axis=0)
    Z_normalized = (Z-Z_mean) / np.sqrt(Z_var)
    return Z_normalized, Z_mean, Z_var